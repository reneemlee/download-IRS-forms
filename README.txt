...
REFLECTION:
In addition to fulfilling the requirements, I also added error handling in response to invalid form numbers and search dates. I also incorprated doctest and unittests to verify my outputs.

Given more time, I'd like to add additional unit tests testing if given a range of valid years, my function will download all forms that exist within that range, with the earliest form revision date matching the minimum year.I'd also add corresponding error messages if the user input begin/end years fall outside of a form's revision dates.
...
To run FIND THE FORMS on your local machine, follow the instructions below(built using Python 3.9.6):
    ...
    Download and open the zipped file: pinwheel.zip
